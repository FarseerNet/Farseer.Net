...
namespace FS.Extends
{
    ...
    public static partial class UtilsExtend
    {
        ...
        {
            var rows  = dt.Select(filterExpression: "", sort: sort);
            var tmpDt = dt.Clone();

            foreach (var row in rows) tmpDt.ImportRow(row: row);
            return tmpDt;
        }
        ...
        public static DataTable Split(this DataTable dt, int pageSize = 20, int pageIndex = 1)
        {
            ...
            for (var i = firstIndex; i < endIndex; i++) dtNew.ImportRow(row: dt.Rows[index: i]);
            return dtNew;
        }
        ...
        public static DataTable Reverse(DataTable dt)
        {
            ...
            for (var i = dt.Rows.Count - 1; i >= 0; i--) tmpDt.ImportRow(row: dt.Rows[index: i]);
            return tmpDt;
        }
        ...
        public static DataTable CloneData(DataTable dt)
        {
            ...
            dt.Rows.ToRows().ForEach(action: newTable.ImportRow);
            return newTable;
        }
        ...
        {
            var lstRow = new List<DataRow>(capacity: drc.Count);
            lstRow.AddRange(collection: drc.Cast<DataRow>());
            return lstRow;
        }
    }
}