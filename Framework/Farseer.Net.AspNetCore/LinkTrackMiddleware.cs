...
public class LinkTrackMiddleware
{
    ...
    public async Task Invoke(HttpContext httpContext, IIocManager _iocManager)
    {
        ...
        using (var trackEnd = FsLinkTrack.TrackApiServer(contextId: contextId, parentAppId: parentAppId, domain: httpContext.Request.Host.Host, path: path, method: httpContext.Request.Method, contentType: httpContext.Request.ContentType, headerDictionary: dicHeader, requestBody: requestContent, ip: httpContext.GetIP()))
        {
            ...
            await using (var responseBody = new MemoryStream())
            {
                ...
                try
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(value: httpContext.Response.ContentType) && (httpContext.Response.ContentType.Contains(value: "json") ||
                        ...
                        trackEnd.SetDownstreamResponseBody(responseBody: rspBody, responseStatusCode: httpContext.Response.StatusCode);
                }
                catch (Exception e)
                {
                    trackEnd.Exception(exception: e);
                    trackEnd.SetDownstreamResponseBody(responseBody: e.ToString(), responseStatusCode: httpContext.Response.StatusCode);
                    throw;
                }
            }
        }
