...
namespace FS.Data.Client.MySql
{
    ...
    public class MySqlBuilder : AbsSqlBuilder
    {
        ...
        internal MySqlBuilder(AbsDbProvider dbProvider, ExpressionBuilder expBuilder, string dbName, string tableName) : base(dbProvider: dbProvider, expBuilder: expBuilder, dbName: dbName, tableName: tableName)
        {
        }
        ...
        public override ISqlParam ToList(int top = 0, bool isDistinct = false, bool isRand = false)
        {
            ...
            if (!isRand)
                ...
                Sql.Append(value: $"SELECT {strSelectSql}{randField} FROM {DbTableName} {strWhereSql} ORDER BY Rand() {strTopSql}");
            else
                Sql.Append(value: $"SELECT * {randField} FROM (SELECT {strDistinctSql} {strSelectSql} FROM {DbTableName} {strWhereSql} {strOrderBySql}) s ORDER BY Rand() {strTopSql}");
            return this;
        }
